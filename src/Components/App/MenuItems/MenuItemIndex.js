...
import { getDataItems, deleteItem } from '../../../Actions/MenuItemActions';
...
const Header = [
  {
    ...
    Cell: row => (
      <div>
        <input 
          name={'name'+row.original.admin_menu_id}
          id={'id'+row.original.admin_menu_id}
          type="checkbox" 
          value={row.original.admin_menu_id}
  ...
  },
  {
    ...
    Cell: row => (
        <div className="text-a-c">
            <Link to={`/tadmin/system/menu-items/edit/${row.original.admin_menu_id}`}> <Button type="button" variant="contained" className="marginR5" color="primary"> <i className="fa fa-edit"></i> </Button> </Link>
        </div>
    )
  }
];
...
class MenuItemIndex extends Component {
    ...
    gotoCreate() {
      this.props.history.push('/tadmin/system/menu-items/create');
    }
    ...
    componentWillReceiveProps(nextProps) {
      ...
      if(nextProps.type == 'MENU_ITEMS') {
        if(nextProps.status){
          this.setState({ data: nextProps.respData.table, pages: nextProps.respData.pages });
        }
      }

      if(nextProps.type == 'DELETE_MENU_ITEMS') {
        if(nextProps.status){
          storeMenuItems(nextProps.respData.menuItems);
          this.setState({ data: nextProps.respData.table, pages: nextProps.respData.pages });
        }
      }
                          ...
                          <CardContent>
                            ...
                            <Grid container spacing={16}>
                                ...
                                <Grid item xs={2} className="text-a-c">
                                  <Button type="button" variant="contained" color="primary" className="marginR5 padding-15" onClick={this.gotoCreate}> <i className="fa fa-plus"></i> </Button>
...
MenuItemIndex.propTypes = {
    classes: PropTypes.object.isRequired,
};

const mapStateToProps = state => ({
    respData: state.menuItem.data,
