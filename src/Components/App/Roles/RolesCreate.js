...
import { createItem, getAllMenus } from './../../../Actions/RolesActions';
...
class RolesCreate extends Component {
    ...
    onSubmit(e) {
        const payload = { role_name: this.state.role_name, menu_list: this.state.menu_list };
    ...
    componentWillReceiveProps(nextProps) {
        if(nextProps.type == 'GET_ALL_MENUS') {
            if(nextProps.status)
                this.setState({ menu_list: nextProps.data})
        }

        if(nextProps.type == 'CREATE_ROLES') {
            if(nextProps.status)
...
export default compose(
    withStyles(classes),
    connect(mapStateToProps, { createItem, getAllMenus })
  )(RolesCreate);